...
function register(app: Express, config: Config) {
    ...
    app.get("/merge", (req, res) => {
        try {
            ...
            mergeCoins(config.ledgerDirectory, parseInt(req.query.origin.toString()), parseInt(req.query.target.toString()), req.query.sign.toString(), parseInt(req.query.target.toString()));
            res.json({ "message": "success" })
        } catch (e: any) {
    ...
    app.get("/split", (req, res) => {
        try {
            ...
            splitCoins(config.ledgerDirectory, parseInt(req.query.origin.toString()), parseInt(req.query.target.toString()), req.query.sign.toString(), parseInt(req.query.target.toString()));
            res.json({ "message": "success" })
        } catch (e: any) {
            res.status(400).json({ "error": e.message });
        }
